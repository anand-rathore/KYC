...
def save_general_info(request):
    try:
        request_fields = ['name', 'contactNumber', 'emailId', 'contactDesignation', 'loginMasterId', 'merchantId',
                          'businessType', 'yourRole']
        ...
        data_savegeneral = saveGeneralInfo(loginMasterId, merchantId, businessType, yourRole, name, contactNumber,
                                           emailId, contactDesignation)

        return Response(data_savegeneral,
                        status=status.HTTP_200_OK if data_savegeneral["status"] else status.HTTP_400_BAD_REQUEST)
